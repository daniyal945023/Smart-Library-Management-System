...
int main()
{
    ...
    if (bookFile.is_open())
    {
        string title, author,bookId, availStr;
    ...
    if (userFile.is_open())
    {
        string name, password, userId, activeStr;
    ...
    while (choice != 8)
    {
        ...
        case 5:
        {
            ...
            {
                if (trackedBooks[i]->getBookId() == bid)
                    foundBook = trackedBooks[i];
            }
        ...
        case 8:
        {
            ...
            for (int i = 0; i < trackedBookCount; ++i)
            {
                 if (!trackedBooks[i] || trackedBooks[i]->getBookTitle().empty()) continue;
                bOut << trackedBooks[i]->getBookTitle() << "|"
                     << trackedBooks[i]->getBookAuthor() << "|"
                     << trackedBooks[i]->getBookId() << "|"
                     << trackedBooks[i]->isBookAvailable() <<endl;
            }
            bOut.close();
            ...
            for (int i = 0; i < trackedUserCount; ++i)
            {
                 if (!trackedUsers[i] || trackedUsers[i]->getUserName().empty()) continue;
                uOut << trackedUsers[i]->getUserName() << "|"
                     << trackedUsers[i]->getUserPassword() << "|"
                     << trackedUsers[i]->getUserId() << "|"
                     << trackedUsers[i]->getUserActiveStatus() <<endl;
            }
            uOut.close();
